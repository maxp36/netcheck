...
def check_config_db(config):
    ...
    if not 'db' in config:
        print('%s "%s" field not defined' % (config_err_text, 'db'))
        exit(1)

    config_db = config['db']
    config_err_text = 'db config error:'
    if not 'host' in config_db:
        print('%s "%s" field not defined' % (config_err_text, 'host'))
        exit(1)
    if not 'user' in config_db:
        print('%s "%s" field not defined' % (config_err_text, 'user'))
        exit(1)
    if not 'pass' in config_db:
        print('%s "%s" field not defined' % (config_err_text, 'pass'))
        exit(1)
    if not 'name' in config_db:
        print('%s "%s" field not defined' % (config_err_text, 'name'))
        exit(1)
...
def check_config_hosts(config):
    ...
    if not 'hosts' in config:
        print('%s "%s" field not defined' % (config_err_text, 'hosts'))
        exit(1)
...
def check_config_host(config):
    ...
    if not 'host' in config:
        print('%s "%s" field not defined in\n %s' %
